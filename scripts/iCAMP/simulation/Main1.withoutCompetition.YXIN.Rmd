...
save.wdm= here("data", "processed", "simulatedData", "NoComp") 
...
tp.file= glue(wdinput, "/YXIN.lowN.tree.pd.rda")
tp.in =  lazyopen(tp.file)
tree=tp.in$tree
pd=tp.in$pd
tree;max(tree$edge.length)
maxpd=max(pd)
op.file= glue(wdinput, "/YXIN.lowN.opens.rda")
ops= lazyopen(op.file)
nworker=8

```

```{r simmulation with various scenario}

## Predefined finction for simulation
simmu<-function(prefix,op,bin.size, save.wdm,...)
{
  save.wd = file.path(save.wdm, prefix)
  if (!dir.exists(save.wd)) {
    dir.create(save.wd, recursive = TRUE)}
  
  # setting gobal parameters
  sampname = c(paste0("Almere_butyrate",1:5),paste0("Bath_butyrate",1:5),
  ...
  ######################
  if(file.exists(glue(save.wdm, paste0("MZSM.meta.J",J/1000/1000,"M.theta",theta,".rda"))))
  {
    MZSM.meta=lazyopen(glue(save.wdm, paste0("MZSM.meta.J",J/1000/1000,"M.theta",theta,".rda")))
  }else{
    MZSM.meta=sads::rmzsm(n=length(op),J=Nk*1000,theta=theta)
    save(MZSM.meta,file=(glue(save.wdm, paste0("MZSM.meta.J",J/1000/1000,"M.theta",theta,".rda"))))
  }
  ...
  s46.sim<-function(a.disp,a.drift,prefixi,...)
  {
    ...
    while(nc.disp!=spn.disp & t<5)
    {
      ...
      save(taxbin,file=glue(wdmm, "/taxabin/", paste0(prefixi,".taxabin.rda")))
...
for(bz in 1:length(bszs))
{
  ...
  simmu(prefix=prefix,op=op,bin.size=bin.size, save.wdm=save.wdm)
}
...
# plot the optimum niche preference
op = as.data.frame(ops$BM$openv)
op_hist = ggplot(op, aes(x = ops$BM$openv)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "gray", color = "white") +
  geom_vline(xintercept = c(0.05, 0.95), color = "red", linetype = "dashed", size = 0.5) +
  labs(
    title = "Distribution of optimum niche preference in simulated data",
    x = "Optimal Position",
    y = "Density"
  ) +
  theme_minimal()
