...
dirpattern="d0" # set some pattern if you want to test the performance with a certain type of simulated situations
...
sim.test<-function(sim.file,wd,...)
{
  ...
  wd=iwd(wd)
  prefix <- paste0(precode, sub(".comm.tree.pd.*.rda$", "", basename(sim.file)))
  print(prefix)
...
iwd(wdm)
prefixss=list.files(path=wdm,full.names = FALSE,recursive = FALSE)
