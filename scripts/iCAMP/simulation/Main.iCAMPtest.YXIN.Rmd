...
code.wd= here("scripts", "iCAMP", "simulation") # a path to the folder you saved the code.
wdm = here("data", "processed", "simulatedData", "NoComp") # to test with the data without competition
...
sim.test<-function(sim.file,wd,...)
{
  ...
  icamp.res=icamp.big(comm=comm,pd.desc=pd.big$pd.file, pd.spname=pd.big$tip.label, 
                      ...
                      ignore.zero=TRUE,output.wd=pd.wd,
...
for(x in 1:length(prefixs))
{
  ...
  STuv=data.frame(Approach="iCAMP",Scenario=senai,ABinSize=abszi,Act.ds=dsi,CompetitionRatio=compri,Situation=Puvm$situation,
                  Level="community", Plot=paste0(Puvm$samp1,Puvm$samp2),STexp=STexp,STest=STest,stringsAsFactors = FALSE)
  
  ccc_result <- epi.ccc(STexp, STest)
  stev <- c(ccc_result$rho.c[,1], ccc_result$rho.c[,2])  # estimate and CI
  #stev=epi.ccc(x=STexp,y=STest,out.vector = TRUE)[1:2]
  STexpk=rowSums(Pgkm[,paste0(c("HD","DL","DR"),".PtExpect"),drop=FALSE])
  STestk=rowSums(Pgkm[,paste0(c("HD","DL","DR"),".PtObs"),drop=FALSE])
  
  #check
  cat("nrow(Pgkm):", nrow(Pgkm), "\n")
  cat("length(STexpk):", length(STexpk), " | length(STestk):", length(STestk), "\n")
  ...
  ccc_result <- epi.ccc(STexpk, STestk)
  stevk <- c(ccc_result$rho.c[,1], ccc_result$rho.c[,2])  # estimate and CI
  #stevk=epi.ccc(x=STexpk,y=STestk,out.vector = TRUE)[1:2]
  ccc_result <- epi.ccc(STexpk*Pgkm$RA.bin, STestk*Pgkm$RA.bin)
  stevkw <- c(ccc_result$rho.c[,1], ccc_result$rho.c[,2])  # estimate and CI
  #stevkw=epi.ccc(x=STexpk*Pgkm$RA.bin,y=STestk*Pgkm$RA.bin,out.vector = TRUE)[1:2]
...
```{r perform iCAMP manually}

sim.file = here("data", "processed", "simulatedData", "NoComp", "BMb3d02", "BMb3d02.S9.Sel75Drift25.comm.tree.pd.ABP.rda")
psdata_YXIN = readRDS(here("data", "processed" , "for_iCAMP", "YXIN_psdata_lowN.rds"))

# assign comm, pd, tree, and ABP from the rda file/object
simd=lazyopen(sim.file)
comm=simd$comm
pd=simd$pd
tree=simd$tree
ABP=simd$ABP
treat = data.frame(sampname = c(paste0("Almere", 1:5), paste0("Bath", 1:5))) %>%
  mutate(sludge_source = str_extract(sampname, "^[A-Za-z]+")
  )
