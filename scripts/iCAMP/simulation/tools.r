...
lazyopen <- function(filepath, load.obj = "all", ...) {
  if (is.null(filepath) || is.na(filepath) || filepath == "") {
    file = NULL
  } else {
    filepath = gsub("\\\\", "/", filepath)
    
    # Make sure filepath is non-NA before regexpr
    if (!is.na(filepath) && regexpr("/", filepath) > 0) {
      filepath = sub("file:///", "", filepath)
      locate = as.vector(gregexpr("/", filepath)[[1]])
      locate = locate[length(locate)]
      file = open.file(filename = substring(filepath, locate + 1),
                       folder = substr(filepath, 1, locate - 1),
                       load.obj = load.obj, ...)
    } else {
      file = open.file(filename = filepath,
                       folder = getwd(),
                       load.obj = load.obj, ...)
    }
  }
